...
export default function Register() {
    ...
    return (
        <div className="min-h-screen bg-surface flex items-center justify-center p-4 relative overflow-hidden font-body">
            ...
            <div className="absolute top-0 left-0 w-96 h-96 ai-gradient opacity-10 rounded-full blur-[100px] pointer-events-none"></div>
            ...
            <div className="w-full max-w-md glass-panel p-10 md:p-14 rounded-[3rem] border border-outline-variant/20 shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-500">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button 
                        type="submit" 
                        disabled={loading}
                        className="w-full mt-4 bg-on-surface text-surface font-black uppercase tracking-[0.2em] text-xs py-5 rounded-2xl shadow-xl hover:bg-primary transition-colors disabled:opacity-50 disabled:animate-pulse flex items-center justify-center gap-3"
                    >
                        {loading ? (
                            <>
